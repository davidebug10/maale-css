...
/* === Merchant Header Video — חומוס אדומים (section 134915) | 2026-06-20 ===
   מזריק וידאו רקע ל-Header של דף החנות הזה בלבד. SPA-safe (MutationObserver).
   העיצוב עצמו ב-global-cdn.css. אם ה-JS לא רץ — נשארת תמונת fallback מה-CSS. */
(function () {
    var VIDEO_URL = "https://davidebug10.github.io/maale-css/hummus-hero.mp4";
    var POSTER = "https://cdn-upload.hyperzod.app/public/6848/images/f1170a0a-3c06-4aa2-9911-2d64ad312b89.webp";

    function injectHummusVideo() {
        var header = document.querySelector('[section-address="134915"] #merchant-header-v2');
        if (!header) return;
        if (header.querySelector('#mh-hummus-video')) return;

        var v = document.createElement('video');
        v.id = 'mh-hummus-video';
        v.src = VIDEO_URL;
        v.poster = POSTER;
        v.muted = true;
        v.defaultMuted = true;
