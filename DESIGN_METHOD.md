...
| מסך הוספת/עריכת כתובת (חלונית במובייל, מגירה בדסקטופ): הדר, שדה החיפוש, רשימת התוצאות, המפה, הטופס, "שמור כתובת" | "חלק 44: MH AddrEditor" (בסוף הקובץ) | + JS `MH Streets` (רחובות שגוגל לא מכיר) + `MH Lang` v1.2.0 (תרגומים). עוגנים: `.scheme-address-editor` (מובייל) / `.v-navigation-drawer .address-panel-swap` (דסקטופ). החליף 22.9 את 12ד, 12ה, 16א–16ה, 21א, 21ב, 23, 28, 29, 29.1, "Fix: Search Results Container Collapse", 26 (תוצאות), 27 (פרימיום). **בלי backdrop-filter/filter/transform על אבות של `.search-results`** (ראו מלכודות) |
