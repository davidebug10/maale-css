...
| כפתורי "הוספה" בכרטיסי מוצר | `html body .v-btn.add-btn …` (~שורה 117) | גלובלי; הפופאפ דורס אותו |
...
**מוצרים בקטגוריה חתוכים ל-20 (10.9, פתוח):** כל קטגוריה מגיעה עם עד 20 מוצרים ודגל `is_paginated`; הבאנדל **לא קורא את הדגל** ואין כפתור "הצג הכל". גם דף הקטגוריה (`/he/m/:slug/:id/category/:cateId`) מציג 20 ולא מביא עוד. ה-API `Search.getProductsByCategory` כן מחזיר `products.next_page_url` — ב"סיגריות חד פעמיות" בגואה יש 40 מוצרים, הלקוח רואה 20. פתרון מיידי בלי קוד: לפצל קטגוריות ל-≤20 מוצרים באדמין. פתרון בקוד (לא נבנה): להביא דפים 2+ ולדחוף ל-`category_products` של הקטגוריה ב-store.
