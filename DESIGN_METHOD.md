...
| רשימת האופציות בפופאפ מוצר | "חלק 22ט: MH Options" (§8 = מגבלת בחירה, 23.9) | + JS `MH Options` + JS `MH OptLimit` (תג "עד N"/"N מתוך M" בכותרת קבוצה מוגבלת, חסימת בחירה מעבר למקסימום, "עד 1" = החלפה; רק בחנויות שב-`MERCHANTS` — כרגע קצפת) |
| פופאפ המוצר (מעטפת, הדר, תמונה, זהות, פוטר) | "חלק 22י: MH Product" | 22א–22ח הוסרו |
| בורר רבעי פיצה | "בורר רבעי פיצה — Pizza Quarters" (`mhq-`) + JS "Pizza Quarters" v1.4 | **לא נוגעים**; יש בדיקה שסופרת שהכללים נשארו. v1.4 (22.9, `mhq-halves-v1`): מסעדה בלי רבע בודד בתפריט מקבלת גלגל של שני חצאים (`.mhq-halves`) — ההחלטה לכל תוספת לפי האופציות שקיימות בפועל |
...
| מסך הוספת/עריכת כתובת (חלונית במובייל, מגירה בדסקטופ): הדר, שדה החיפוש, רשימת התוצאות, המפה, הטופס, "שמור כתובת" | "חלק 44: MH AddrEditor" v1.0.3 (בסוף הקובץ) | + JS `MH Streets` (רחובות שגוגל לא מכיר) + `MH Lang` v1.2.0 (תרגומים). עוגנים: `.scheme-address-editor` (מובייל) / `.v-navigation-drawer .address-panel-swap` (דסקטופ). החליף 22.9 את 12ד, 12ה, 16א–16ה, 21א, 21ב, 23, 28, 29, 29.1, "Fix: Search Results Container Collapse", 26 (תוצאות), 27 (פרימיום). **בלי backdrop-filter/filter/transform על אבות של `.search-results`** (ראו מלכודות) |
...
| חלונית "עוד אחד עם אותן תוספות?" (`#AddonsConfirmation`, נפתחת מ-+ על מוצר עם תוספות שכבר בעגלה) | "חלק 49: MH Repeat" (בסוף הקובץ) | + JS `MH Repeat` (תרגום: "הבחירות הקודמות שלך"→"להוסיף עוד אחד עם אותן תוספות?", "חזור"→"כן, אותו דבר", "בחירה"→"לבחור מחדש"; מוריד את הפסיק אחרי כל תוספת) |
| סרגל ניווט תחתון | "חלק 31" | |

### 1.2 בלוקי ה-JS ב-`custom-footer.js`

כל בלוק: IIFE, `'use strict'`, שומר ריצה-אחת (`window.__MH_X__`), נכשל-פתוח (try/catch, בלי לזרוק), חושף `window.MH_X = { version, stats() }` לבדיקות. רשימה: בחירת ישראל בכתובת · MH-ZONE (אזור משלוח) · MH Preorder (הזמנה מראש) · MH SEO · Pizza Quarters (`__mhq`) · MH Pay (סימון אמצעי תשלום נבחר) · MH PayIntent (מילוי country בבקשת התשלום) · MH Pay NoPreselect (ביטול סימון אוטומטי של אמצעי התשלום האחרון) · MH Search (טקסטים בדף החיפוש) · MH Options (`mh-opt-on` על אופציה נבחרת) · MH CatNav (`swiper.update()`) · MH CatMap (מפת קטגוריות, v1.1.2 9.9) · MH MenuLoad (טעינת כל דפי התפריט, v1.0.0 10.9) · MH ShowMore ("הצג עוד" בקטגוריה חתוכה, v1.0.1 10.9) · MH KafitAddr (הדגשת שדה הכתובת בקפית, v1.0.0 10.9) · MH Lang (תרגום מחרוזות שחסרות בחבילת השפה — "Customizable"→"ניתן להתאמה", "Phone:"→"טלפון:"; v1.1.0 10.9) · MH AgeGate (`window.MH_AGEGATE`, שער 18+; v1.2.0 9.9). שתי דרכי חסימה מצומצמות: (א) `RESTRICTED` — 16 שמות קטגוריה **מדויקים** (התאמה מלאה, לא הכלה), עובד גם בדף החיפוש; (ב) `AGE_MERCHANTS` — חנויות שכל המלאי בהן 18+, לפי **מזהה החנות ב-URL** בלבד (כרגע גואה `6aa076d67a4248e3ff053582`). (ב) שורד שינוי שמות קטגוריות אצל המסעדן ולא יכול לדלוף לחנות אחרת. · MH Streets (רחובות מעלה אדומים שגוגל לא מכיר — טבלת GovMap, הזרקה לתוצאות חיפוש הכתובת; `window.MH_STREETS.find(q)`) · MH WhatsApp (בועת וואטסאפ; `window.MH_WA.stats()`) · MH OptLimit (מגבלת מינימום-מקסימום בקבוצת תוספות מתוך נתוני getById — `enable_range` + `max_quantity`; Hyperzod לא אוכפת את זה בפופאפ — אומת 23.9 שנכנסו לעגלה 5 טעמים במקום 3. הספירה נשמרת לפי שם אופציה כי "הראה פחות" מסיר מה-DOM אופציות מסומנות; `window.MH_OPTLIMIT.stats()`) · MH ILPhone · MH Repeat (טקסטים בחלונית "עוד אחד עם אותן תוספות?") (טלפון בטופס הכתובת: ישראל תמיד, בוחר "Israel" ברשימת Hyperzod דרך li.click(); הנעילה ב-CSS חלק 44; `window.MH_ILPHONE.stats()`)
...
**חלונית "בחר כתובת" (10.9):** רכיב חדש בבילד `index-C04UkX5H`. עוגן ייחודי `#SelectAddress` (גם `.scheme-address-select-panel`), מרונדר רק בתוך `address-card` בצ'ק-אאוט — **לא ניתן לפתוח אותו מדף הבית** (`sidePanel.addressList` לבדו לא מספיק), ולכן המוק נבנה מתבנית הרינדור בבאנדל (`function T8`). מבנה: `.v-card-title` → `.scheme-address-select-add > .add-address-btn` → `h3` → `.v-card-text.custom-scroll > div[data-test-id^="test-id-"]` לכל כתובת, עם `.scheme-address-select-icon`, `span.tw-text-[9px]` (מרחק), `span.tw-text-[14px]` (שם), `.scheme-status-badge--success` (הכתובת הפעילה) ו-`p` (הכתובת). הבלוק הגנרי `.v-bottom-sheet__content .v-card{padding:24px 20px}` דוחס אותה — מנוטרל לוקאלית ב-41, כמו ב-22י. **"Phone:" לא בחבילת השפה** ומגיע כטקסט ישיר של אותו אלמנט שמכיל את המספר, לכן `font-size:0` היה מעלים גם את המספר; ההחלפה נעשית ב-MH Lang (החלפת תחילית בטקסט, רק באלמנט עלה).

**סוגי הזמנה ללא כתובת (10.9):** `boot.order_types_tenant[].requires_address` קובע אם מוצג כרטיס הכתובת. לקפית שני הסוגים (`pickup`, `custom_1`) הם `false`, ולכן **אין לה כרטיס כתובת בכלל** והלקוח כותב את הכתובת בשדה טקסט חופשי שהמסעדן הגדיר. הכיוון הנכון לתקן בשורש: להפוך את סוגי ההזמנה שלה ל-`requires_address:true` באדמין. **הצ'ק-אאוט לא נגיש בלי התחברות** — כאורח מגיעים אליו עם עגלה דרך `$router.push({name:'checkout'})`, אבל שדות שתלויים במשתמש מחובר לא מרונדרים. לשדות כאלה: לזהות ב-JS לפי **טקסט** (לא לפי מחלקה/id של Hyperzod, שאין להם), לתייג במחלקה משלנו, ולעצב אותה — כמו MH KafitAddr. הסקופ הכפול: `#checkout` קיים **וגם** `getCartMerchant` שווה למזהה העסק.
...
**מחשב 8GB (23.9):** בדיקות ללא ראש על החנות החיה (סרטוני הירו + תמונות) הקפיאו את המק כשרצו כמה כרומים ברצף. לחסום ב-`Network.setBlockedURLs` קבצי mp4/jpg/png/webp ותמונות drive, כרום אחד בכל פעם, ו-`pkill -f .profiles/<name>` ביציאה מהסקריפט (גם בקריסה).

**מלכודת (22.9) — JSON-LD לא נכתב בבלוק Custom HTML:** בלוק MH SEO (`custom-footer.js`) מייצר בעצמו `#mh-ld-merchant` בתוך ה-`<head>` מרשומת העסק (שם, טלפון, כתובת, קואורדינטות) ומוחק כל `mh-ld-*` שלא שייך לדף הנוכחי. סקריפט `application/ld+json` שמודבק ידנית בבלוק Custom HTML נמחק בטעינה (נמדד במוק: `getElementById` מחזיר null). לכן בלוקי ההירו/SEO מכילים רק HTML+CSS. נבדק שהעמוד החי של קצפת כבר מפרסם JSON-LD מלא בלי שנגענו.

**תבנית בלוק ההירו/SEO של עסק (22.9):** Hyperzod עוטפת כל בלוק ב-`div.CustomHTML--<id>` ומקדימה את הסלקטורים שלנו אוטומטית — לכן כותבים סלקטורים רגילים `#mh-hero-<slug>` / `#mh-about-<slug>` עם `!important`, בלי להמציא את מספר הבלוק. ההירו: `.mh-hero-box` בגובה `min(75vw,560px)`, וידאו `object-fit:cover` עם `class="mhh-video"` (ה-kick של הפוטר מפעיל אותו), טקסט על גרדיאנט שחור מלמטה (kicker/title/p/line). ה-SEO: כרטיס לבן `.mh-about-box` עם פסקה אחת. קו המבטא בצבע המותג.

**מלכודת (22.9) — שני ריפואים, מנגנון אחד:** התפריט של פיצרייה (maale-ops, `buildPizzaTopping`) והגלגל (maale-css, "Pizza Quarters") הם שני צדדים של אותה קונבנציית שמות `"תוספת (הכל|חצי ימין|חצי שמאל|רבע …)"`. כשמחקו ב-maale-ops את הרבעים של שיבולת השרון (העסק מוכר חצאים מינימום), הגלגל החי המשיך לצייר ארבעה רבעים: לחיצה על רבע → "הצירוף הזה לא מוגדר", והתווית "₪2 לרבע" חושבה כרבע מ-₪8 שאי אפשר לקנות. מאז v1.4 הגלגל נגזר מהאופציות שקיימות (רבע בודד → 4 רבעים; רק חצי ימין/שמאל → 2 חצאים). כלל: שינוי שמות/מבנה תוספות פיצה בתפריט = לבדוק את הפופאפ החי עם `__mhq.stats()` (`cards`, `halves`) לפני שמדווחים "בוצע". מוקים אמיתיים לשני המצבים נלכדים עם `__mhq.destroy()` ואז `outerHTML` של `.product-popup`.
...
- **חלונית "עוד אחד עם אותן תוספות?" (24.9):** רכיב `cart-item-addon-confirm` (ב-chunk dialog-*.js), `sidePanel.ItemAddonConfirm`, נפתח מלחיצה על `.increment-btn` בכרטיס מוצר עם תוספות שכבר בעגלה. DOM: `#app-cart-addon-confirm.v-overlay.v-bottom-sheet` (z 99999, width 600) > `.v-overlay__content.v-bottom-sheet__content` > `#AddonsConfirmation.v-sheet` > `.v-card` > `.v-card-title.product-name` (שם המוצר) + `.v-card-text` > `.text-subtitle-2` (הכותרת) + `.product-addons` > `span` > `span` ("שם, " לכל אופציה — הפסיק בטקסט) [+ `.tw-mt-2` הערת מוצר | `p.tw-text-gray-600` "No customizations added"] + [`.tw-mt-3` אזל מהמלאי] + `.v-card-actions > .v-row > .v-col` ×2: `.chose-btn` (outlined, פותח את הפופאפ מחדש) ואז `.repeat-ntn` (flat שחור, `addToCart inc 1`). הטקסטים מ-`getLug().cartProductRepeat` (חבילת השפה של Hyperzod — התרגום שם שגוי). מודל בדיקה: `flow.mjs` — פלוס → רדיו ראשון בכל קבוצה → "הוספה" → `.increment-btn`.
...
### 6.7ב מסך הוספת/עריכת כתובת (`sidePanel address` / `editAddress`)
מובייל: `.v-overlay.v-bottom-sheet.v-dialog--fullscreen` → `.v-overlay__content.v-bottom-sheet__content` (contain:layout) → `.v-card.scheme-address-editor` → `.scheme-address-editor__header` (`.tw-h-[56px]` עם `img[alt="Back"]` + `span.tw-truncate` "הוסף כתובת חדשה"; מתחתיו `.tw-px-4 > .scheme-location-master > .v-card.l-search > .v-input.mobile-search-input > .v-field > input#search` + `.search-icon-mobile.scheme-colorable-icon--search|--close`) · `.search-results.tw-fixed` (heading `.scheme-location-results-heading`, `.v-card.results-list`, שורות `.scheme-location-result` עם `__title`/`__address` ב-innerHTML) · `.v-card-text.scheme-side-panel__surface` (הגלילה) → `#locationConfirm.scheme-address-editor__content` → מפה `.tw-h-[346px]` (`#Map`, בועה `.tw-bg-[#212121]`, `#CurrentLocationButton`), ידית `.tw-bg-[#e5e5e5]`, כותרת `h2` + `p.line-clamp-2`, `form#addAddressForm` (`#AddressInputBuilding`, `#AddressInputLandmark`, `#AddressInputContactPhoneNumber > #login-input` עם `.country-select`, `#AddressInputCountry` (מוסתר), `#AddressSelectType` צ'יפים, `#AddressInputContactName`) · פוטר `.scheme-address-editor__mobile-footer > button.scheme-address-editor__submit`. דסקטופ: `nav.v-navigation-drawer--right` → `.address-panel-swap` (480px). ה-hash-ים הישנים (bd9430d0, 4b9952fb, d055d74b, 3b7172a6) לא קיימים יותר.
...
**חיפוש כתובת — רחובות חדשים לא נמצאים (22.9, לקוח אמיתי: "הפסנתר", "הר הלבונה"):** חיפוש הכתובת של Hyperzod = Google Places דרך השרת שלהם (`GET /store/v1/places/search`, `searchable_countries:["IL"]`), עם race של 2 שניות ונפילה שקטה ל-Mapbox (`api_keys.map_keys.mapbox`). ל-Google (וגם ל-Mapbox ול-OSM) אין את הרחובות החדשים של מעלה אדומים: ביקורת מול 167 הרחובות הרשמיים (data.gov.il, סמל ישוב 3616) — ~20 חסרים. הפתרון: `MH Streets` — טבלה מקומית [שם, lat, lng] מה-GovMap של מפ"י (`POST https://www.govmap.gov.il/api/search-service/autocomplete`, גוף `{searchText, language:'he', isAccurate:false, maxResults}`, CORS פתוח, מחזיר Web Mercator EPSG:3857 → WGS84). הבלוק עוטף את `store.dispatch('searchLocation')` ומוסיף תוצאה בראש כשגוגל לא החזירה שורת רחוב (וכשגוגל החזירה "No results" — מחזיר הצלחה, אחרת הרכיב בורח ל-Mapbox ודורס), ועוטף גם את `fetch` של Mapbox (הנתיב השקט אחרי כישלון/2 שניות) ומקדים features — v1.0.1, נמדד חי בשלושת הנתיבים; הבחירה עוברת בנתיב ה-`mapbox_data` הקיים של הרכיב (`fetchPlace` קורא `center` + `context`) — בלי רשת ובלי reverse-geocode שדורס את הטקסט (נמדד: אחרי הבחירה `getMapLocation.address` = הטקסט שלנו). כל 146 הנקודות בתוך פוליגון MH-ZONE (נבדק `MH_ZONE.check(lat,lng)`). ריענון: `node tools/streets-govmap.mjs > work/streets.js`. 4 רחובות בלי מיקום בשום מפה (הזוגן, החורן, החלמונית, השרון) — מחכים לקואורדינטה מדוד.

**מסך הוספת כתובת — רשימת התוצאות היא `position:fixed` עם `top` inline (22.9):** הרכיב `location-master` (data-v-7cfbf9c1, `.scheme-location-master`) מחשב `stickyResultsTop = תחתית שדה החיפוש` ומציב את `.search-results.tw-fixed` מ-top עד bottom:0 עם `tw-overflow-y-auto`. כל `backdrop-filter` / `filter` / `transform` (וגם `contain`) על **אב** של הרשימה הופך אותו ל-containing block — וה-top נמדד ממנו במקום מהמסך. הכלל הגנרי של הגיליון (`.v-bottom-sheet__content .v-card { backdrop-filter: blur(50px) }`) עשה בדיוק את זה, ובלוקים ישנים "תיקנו" עם `margin-top:-88px` + `overflow:hidden` — שחתכו את ה-× ואת הגלילה. חלק 44 מנטרל את הפילטרים על הכרטיסים בסקופ ולא נוגע ב-top. הבדיקה: `.search-results` נמדד מתחת ל-`.v-input` של השדה (פער 0–12px) וניתן לגלילה.

**חיפוש כתובת בכרום ללא ראש (22.9):** `Input.insertText` מעדכן את ה-v-model אבל **לא** מפעיל את החיפוש — הדיבאונס של Hyperzod (`v-debounce … 350ms`) מאזין ל-keyup. מקלידים עם `Input.dispatchKeyEvent` (keyDown+keyUp לכל תו). חלונית "הוספת כתובת" נפתחת גם כאורח: `store.commit('sidePanel',['address',true])` (מובייל: `.v-bottom-sheet` מלא-מסך עם `.v-card.scheme-address-editor`; דסקטופ: `nav.v-navigation-drawer` עם `.address-panel-swap`, ובו `#search` נסתר עד לחיצה על "שינוי"). מודאל המיקום בדף הבית (`sidePanel searchModal`, `#mobileLocationSearch`) הוא אותו רכיב במצב אחר — בקרה שחלק 44 לא זולג אליו.
...
| `transform:none !important` על עוגן שמופיע גם על ה-`<nav class="v-navigation-drawer">` (23.9, חלק 44 v1.0.2) | בדסקטופ המגירה הסגורה של הוספת כתובת ("Add New Address") נתקעה גלויה בצד ימין **בכל דף** — Vuetify מסתירה מגירה סגורה עם `translateX(480px)` inline, ו-!important ניצח אותו | לעולם לא `transform`/`filter`/`backdrop-filter` על סלקטור שיכול לתפוס מגירה (`.scheme-address-editor` יושב גם על ה-nav בדסקטופ). לעגן על הכרטיס: `.v-card.scheme-address-editor`. בדיקת חיים לדסקטופ: לכל `nav.v-navigation-drawer` ללא `--active` — `getComputedStyle(n).transform` שווה ל-`n.style.transform` |
| מספר טלפון/וואטסאפ שנקרא מה-boot של Hyperzod (24.9, MH WhatsApp v1.0.0) | Hyperzod שומרים את כל ה-Vuex ב-`localStorage.vuex` (כולל `business_details.business_phone`). לקוחה חוזרת קיבלה את ה-state הישן עם המספר הפרטי של דוד, והבועה קישרה אליו. ה-boot הטרי מחליף את המספר רק אחרי כמה שניות (ובאפליקציה שנשארת פתוחה — לא בכלל) | **מספרי טלפון, מיילים וכל דבר שדוד מחליף — קבועים בקוד שלנו, לא מה-boot.** שוחזר: `localStorage.vuex` עם מספר ישן + חסימת `/store/v1/boot` → הבועה החיה קישרה למספר הישן. אחרי החלפת מספר: לעדכן `NUMBER` ב-MH WhatsApp וגם את דף "דברו איתנו" (HTML) |
| "הוראות מוצר" / "הערות מיוחדות למנה" (25.9) | שדה טקסט חופשי בפופאפ המוצר (ובחלונית "עוד אחד") — מגיע מהגדרה **פר-מוצר** בפאנל הסוחר: עריכת מוצר → "הפעל הוראות מוצר" (+ "הפוך לשדה חובה"). דוד לא רוצה אותו (יש הערות למטבח בצ'ק-אאוט). היה דלוק על מוצר אחד בלבד (סכו"ם חד"פ, חומוס אדומים) וכובה שם. | לא להסתיר ב-CSS — אם השדה "חובה" ההסתרה חוסמת הוספה לעגלה. לבדוק מי דלוק: לסרוק את ה-Vuex בדף עסק אחרי גלילה לתחתית (MenuLoad) — `product_instruction_enabled===true`. פאנל סוחר: מסך Merchants → "Login" בשורת הסוחר → `/m/<merchantId>/products` → עיפרון |
